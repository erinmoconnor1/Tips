...
import { getTipAdvisor } from "../graphql/queries";
import { updateTipAdvisor } from "../graphql/mutations";
const client = generateClient();
export default function TipAdvisorUpdateForm(props) {
  const {
    id: idProp,
    tipAdvisor: tipAdvisorModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = tipAdvisorRecord
      ? { ...initialValues, ...tipAdvisorRecord }
      : initialValues;
    setDate(cleanValues.Date);
    setEmployees(cleanValues.Employees);
    setFRIENDLINESS_ATTITUDE(cleanValues.FRIENDLINESS_ATTITUDE);
    setTIMELINESS_OF_SERVICE(cleanValues.TIMELINESS_OF_SERVICE);
    setTABLE_MAINT(cleanValues.TABLE_MAINT);
    setDINING_EXP(cleanValues.DINING_EXP);
    setTotalCost(cleanValues.totalCost);
    setTotalTip(cleanValues.totalTip);
    setErrors({});
  };
  const [tipAdvisorRecord, setTipAdvisorRecord] =
    React.useState(tipAdvisorModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getTipAdvisor.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTipAdvisor
        : tipAdvisorModelProp;
      setTipAdvisorRecord(record);
    };
    queryData();
  }, [idProp, tipAdvisorModelProp]);
  React.useEffect(resetStateValues, [tipAdvisorRecord]);
  const validations = {
    Date: [],
    Employees: [],
    FRIENDLINESS_ATTITUDE: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          ...
          FRIENDLINESS_ATTITUDE: FRIENDLINESS_ATTITUDE ?? null,
          TIMELINESS_OF_SERVICE: TIMELINESS_OF_SERVICE ?? null,
          TABLE_MAINT: TABLE_MAINT ?? null,
          DINING_EXP: DINING_EXP ?? null,
        ...
        try {
          ...
          await client.graphql({
            query: updateTipAdvisor.replaceAll("__typename", ""),
            variables: {
              input: {
                id: tipAdvisorRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TipAdvisorUpdateForm")}
      {...rest}
    >
      <TextField
        label="Date"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || tipAdvisorModelProp)}
